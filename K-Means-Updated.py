...
class EnhancedPJPClustering:
    ...
    def prepare_clustering_features(self, df, iteration=0):
        self.logger.info("Preparing clustering features with dynamic weights...")
        ...
        if var_geo != 0 and var_workload != 0:
            geography_weight = (1 / var_geo) * (1.1 ** iteration)  # Weight geography more in later iterations
            workload_weight = (1 / var_workload) * (0.9 ** iteration)  # Weight workload less in later iterations

            # Normalize weights
            total_weight = geography_weight + workload_weight
            geography_weight /= total_weight
            workload_weight /= total_weight
            self.logger.info(f"Dynamic weights calculated: geography_weight={geography_weight:.4f}, workload_weight={workload_weight:.4f}")
        else:
        ...
        self.logger.debug("Clustering features prepared with dynamic weights.")
    ...
    def refine_with_travel_time(self, df, max_cluster_travel_time=120):
        ...
        while True:
            ...
            for cluster_id in clusters_to_split:
                ...
                # Step 1: Split cluster based on centroid proximity
                self.logger.info(f"Splitting cluster {cluster_id} based on centroid proximity...")
                cluster_data = self.split_clusters_by_centroid_proximity(cluster_data)
                
                # Update cluster labels back to the main dataframe
                df.loc[cluster_data.index, 'cluster'] = cluster_data['cluster']

            # Step 2: Reduce overlap between clusters
            self.logger.info("Reducing overlaps between clusters...")
            df = self.reduce_cluster_overlap(df)

            # Increment iteration for dynamic weight adjustments and continue refinement
            iteration += 1
